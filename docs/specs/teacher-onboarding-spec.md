...
| `approved` | 老師已通過審核，可進入 marketplace 核心流程。 | **已落地**（`teacher-profile-edit` 已確認）：Teacher 可在 `/teacher/profile` 維護 profile（`displayName`／`bio`／`teachingStyle`／`experienceYears`／`specialties`／`serviceAreas`／`teachingFormats`／`certifications`／`priceRange`／`profilePhotoUrl`）與 availability（`/teacher/availability`，`teacher-availability` 已確認）。「會影響公開呈現或媒合判斷的重大欄位變更，未來可再定義是否需重新 review」這句話本輪明確拍板：**V1 不做**——編輯不觸發重新審核、不改變 `status`、不新增 notification，理由與 Admin 端的既有補救手段（唯讀查看老師目前完整欄位內容）見 `docs/domain/data-model.md` 的 TeacherProfile Edit 說明；未來若真的需要重新審核機制，仍是一個獨立、待另行定義的產品決策。 | `suspend`。 | 可公開顯示在 V1 允許的老師展示場景。 | 可查看 eligible demand requests 並提交 response。 | 需通知 Teacher 已通過，可開始使用 teacher dashboard 與需求回應能力。 |
