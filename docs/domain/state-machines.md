...
**V1 落地範圍（`organizer-demand-request-foundation` D9、`demand-response-selection-and-matching` D1/D2/D4 已確認）**：上述完整狀態機是 marketplace 的最終設計，但目前只**接線**以下子集：
...
`under_review`、`teacher_responded`、`converted_to_class`、`completed`、`cancelled`、`expired` 這些狀態值在 Prisma enum 中**保留**（避免未來相關 slice 需要再次 enum migration），但**不提供**對應的 transition 或 UI 動作：
...
- Class conversion（`matched → converted_to_class` 之後）屬未來獨立 plan（demand-to-class conversion），不在目前 scope。
