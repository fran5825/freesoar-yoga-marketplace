...
#### 13b. 測試資料污染開發資料庫（已處理 2026-09-26）

- 調查結果：殘留的 50 個測試帳號（email 結尾 `.local`）全是 7/31、8/2 的舊資料，最近的全套測試沒有留下任何新資料，所以現行測試的清理機制是好的。
- 已處理：一次性刪除這 50 個帳號與其老師資料 22、團主資料 13、團體 13、需求 14、課程 8、評價 4 等（整批在同一個 transaction 內，刪前刪後核對過真實帳號資料筆數不變）。現在資料庫只剩 `fran.zheng@gmail.com`、`pattern178@gmail.com` 兩個帳號與其資料。
- 之後若又看到「Organizer chromium-…」這類假資料，代表有新的測試沒清乾淨，再回來查。
